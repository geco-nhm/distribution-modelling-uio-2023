...
# 1. Lilioceris lilii - a beetle
# 2. Oryctolagus cuniculus - a mammal
# 3. Auricularia auricula-judae - a fungus
# 4. Hesperis matronalis - a plant
...
r.list <- list.files("lab_data/RTPD/TIFF_EXTENT/NORWAY", pattern="tif$", full.names=TRUE) # modify path and/or working directory if needed
...
key <- name_backbone(name= "[WRITE YOUR SPECIES NAME HERE]")$speciesKey
...
training.data.Norway <- generate.training.data(sp.Norway,predictors.Norway,generate.absences=TRUE,absence.value=NA,n.abs=1000,factors=c("ar50.artype.2km","geo.norge123"))   # To see how the function 'generate.training.data' works, open the file 'Dataprep.functions.r'
